...
import { validatecelular, validateDocumentNumber } from "../utils/validation";
...
export const useForm = () => {
  ...
  const handleSubmitForm = async () => {
    const isValid =
      ...
      formInfo.celular.value !== "" &&
      formInfo.celular.isValid;
    ...
    if (isValid && policiesAcepted) {
      ...
      const celular = formInfo.celular.value;

      const userDataResponse = await getUser(documentNumber, celular);
